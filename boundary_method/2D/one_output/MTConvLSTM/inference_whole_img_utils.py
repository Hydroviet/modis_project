...
from inference import InferenceMTConvLSTM
from gen_data_inference_utils import gen_boundary_patch, select_data


class InferenceMTConvLSTMWholeImg:

    def __init__(self, params):
        self.params = params
        self.patch_size = params['img_width']
        self.img_width = params['whole_img_width']
        self.inference_mt_convlstm = InferenceMTConvLSTM(params)


    def _get_inference_from_np_array_patch(self, inputs_np_patch):
        return self.inference_mt_convlstm.get_inference_from_np_array(inputs_np_patch)
