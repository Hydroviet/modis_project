...
def _conv_op(conv_out, w, b, thread_id, k, return_dict):
    x = None
    for i in range(thread_id*k, (thread_id + 1)*k):
        _x = conv_out[:, i:i+1, :]
        _w = w[i, :,:]
        conv_out_i = K.dot(_x, _w)  # output_shape = [num_samples, 1, output_dim]
        if b is not None:
            _b = b[i, :]
            conv_out_i = K.bias_add(conv_out_i, _b)
        x = conv_out_i
        #f = dill.dumps(conv_out_i)
        #return_dict[i] = f
    print(type(x))

def _recurrent_conv_op(conv_out, w, thread_id, return_dict):
    _conv_op(conv_out, w, None, thread_id, k, return_dict)
    '''
    for i in range(thread_id*k, (thread_id + 1)*k):
        _x = conv_out[:, i:i+1, :]
        _w = w[i, :, :]
        recurrent_conv_out_i = K.dot(_x, _w)  # output_shape = [num_samples, 1, output_dim]
        return_dict[i] = recurrent_conv_out_i
    '''

def deserialize(return_dict):
    res = []
...
class GraphConvLSTM_1(GraphConvRecurrent):
    """Convolutional LSTM.
    ...
    # @interfaces.legacy_GraphConvLSTM_1_support  # TODO: include legacy support
    def __init__(self, units,
                 graph_conv_tensor,
                 activation='tanh',
                 recurrent_activation='hard_sigmoid',
                 ...
                 **kwargs):
        super(GraphConvLSTM_1, self).__init__(units,
                                              graph_conv_tensor,
                                              return_sequences=return_sequences,
                                              go_backwards=go_backwards,
                                              stateful=stateful,
        ...
        self.k = self.num_nodes//self.n_threads
        self._conv_out_list = None
        self._recurrent_conv_list = None
    ...
    def build(self, input_shape):
        ...
        kernel_shape = (self.num_nodes, (self.poly_degree + 1) * self.input_dim, self.units * 4)
        self.kernel_shape = kernel_shape  # output_shape = [input_dim * K, output_dim * 4]
        recurrent_kernel_shape = (self.num_nodes, (self.poly_degree + 1) * self.units, self.units * 4)
        self.recurrent_kernel_shape = recurrent_kernel_shape  # output_shape = [output_dim * K, output_dim * 4]
        ...
        if self.use_bias:
            self.bias = self.add_weight(shape=(self.num_nodes, self.units * 4,),
                                        initializer=self.bias_initializer,
                                        name='bias',
                                        regularizer=self.bias_regularizer,
                                        constraint=self.bias_constraint)
            if self.unit_forget_bias:
                bias_value = np.zeros((self.num_nodes, self.units * 4,))
                bias_value[:, self.units: self.units * 2] = 1.
                K.set_value(self.bias, bias_value)
        else:
            self.bias = None
    ...
    def input_conv(self, x, w, b=None):
        # x = [num_samples, num_nodes, input_dim]
        # w = [num_nodes, input_dim * K, output_dim]
        # graph_conv_tensor = [K, num_nodes, num_nodes]
        ...
        processes = [mp.Process(target=_conv_op, 
                                args=(conv_out, w, b, thread_id, self.k, return_dict))
        ...
        self._conv_out_list = deserialize(return_dict)
        conv_out = K.concatenate(self._conv_out_list, axis=1) # output_shape = [num_samples, num_nodes, output_dim]
        return conv_out  # output_shape = [num_samples, num_nodes, output_dim]
    ...
    def reccurent_conv(self, x, w):
        ...
        processes = [mp.Process(target=_recurrent_conv_op, 
                                args=(conv_out, w, thread_id, self.k, return_dict))
                 for thread_id in range(self.n_threads)]
        for p in processes:
            p.start()
        for p in processes:
        ...
        self._recurrent_conv_list = deserialize(return_dict)
        conv_out = K.concatenate(self._recurrent_conv_list, axis=1) # output_shape = [num_samples, num_nodes, output_dim]
        return conv_out  # output_shape = [num_samples, num_nodes, output_dim]
    ...
    def get_config(self):
        config = {'activation': activations.serialize(self.activation),
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(GraphConvLSTM_1, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
