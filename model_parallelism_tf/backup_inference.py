...
class ECLM:
    ...
    def inference(self):
        ...
        for device, idx in zip(self.devices, self.block_idx_per_gpu):
...
def train(eclm, train_filenames, val_filenames, server, init, train_epochs=3, batch_size=1, keep_rate=1.):
    initial_step = 1
...
def inference_all(sess, eclm, test_filenames, batch_size=1, keep_rate=1.0, inference_dir=None):
    ...
    steps = math.ceil(n_tests/batch_size)
    test_losses = []
    VERBOSE_STEP = eclm.verbose_step
...
def arg_parse():
    ...
    parser.add_argument("--num_blocks", default=2, type=int, help="depth of network")
    parser.add_argument("--batch_size", default=1, type=int, help="batch size")
    parser.add_argument("--train_epochs", default=50, type=int, help="train epochs")
    parser.add_argument("--lr", default=0.001, type=float, help="starter learning rate")
...
def main():
    ...
    if mode == 'train':
        ...
        train(eclm, train_filenames, val_filenames, server, init, train_epochs, batch_size)
    else:
        eclm = ECLM(in_steps, out_steps, img_shape, devices, filters,
        ...
        with tf.Session(server.target) as sess:
            ...
            #inference_dir = os.path.join('inference', str(out_steps), 'test')
            #inference_all(sess, eclm, test_filenames, inference_dir=inference_dir)

            inference_dir = os.path.join('inference', str(out_steps), 'train')
            inference_all(sess, eclm, train_filenames, inference_dir=inference_dir)
